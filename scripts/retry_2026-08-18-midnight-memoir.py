# -*- coding: utf-8 -*-
"""
重试脚本：添加IP 183.158.16.77 到微信公众号白名单后运行此脚本。
文章：凌晨三点的便利店，治愈了我一整年的孤独
用法：python D:/blog/scripts/retry_2026-08-18-midnight-memoir.py
"""

import json, os, re, sys, time, urllib.request

APP_ID_FILE = r"D:\blog\scripts\.env"
COVER_PATH = r"D:\blog\content\posts\cover_raw_xiaozingzi.png"
ARTICLE_PATH = r"D:\blog\content\posts\2026-08-18-midnight-memoir.md"
TITLE = "凌晨三点的便利店，治愈了我一整年的孤独"
DIGEST = "深夜的便利店灯永远亮着，城市的万家灯火里，总有一盏是为你而亮的。"
...
def md_to_html(md_text):
    ...
    for line in lines:
        ...
        if s.startswith("> **") and s.endswith("**"):
            ...
            html.append(f'<p {style}><strong style="font-size:18px;color:#555;">{text}</strong></p>')
            continue
        if s.startswith("> "):
            text = s[2:].strip()
            html.append(f'<blockquote style="border-left:4px solid #ddd;padding-left:16px;margin:16px 0;color:#666;font-style:italic;">{text}</blockquote>')
            continue
        if s.startswith("## "):
            text = s[3:]
            html.append(f'<h2 style="font-size:20px;color:#333;margin:32px 0 16px;border-left:4px solid #c0392b;padding-left:12px;">{text}</h2>')
            continue
        if not s:
            html.append("")
            continue
...
def main():
    ...
    with open(r"D:\blog\scripts\.token_cache.json", "w") as f:
        json.dump({"access_token": access_token, "expires_at": time.time() + 7000}, f)
